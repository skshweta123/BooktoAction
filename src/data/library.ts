  ...
  "atomic-habits": {
    productivity: {
      ...
      plan: [
        ...
        "  - Midday Correction (3 minutes): If morning block fails, run immediate 15-minute fallback block before 3 PM.",
...
function buildGoalResult(book: Book, goalId: GoalId): GoalResult {
  ...
  const framework = enforceAnchors(
    [
      ...
      "  - Reader Takeaway: Use these principles as behavior mechanics, not as abstract theory.",
      "---",
      `${derivedFrameworkName}`,
      "Framework Intent:",
      `  - Convert ${book.title} principles into a concrete system to ${strategy.objective}.`,
      "Pillar 1 (Trigger Design):",
      `  - Setup: Build predictable start conditions around ${strategy.weeklyPriority}.`,
      `  - Decision Rule: If '${strategy.riskPattern}' appears for 2 consecutive days, simplify start scope by 50%.`,
      "Pillar 2 (Execution Block):",
      `  - Setup: Run one protected block daily with clear output definition aligned to ${focus}.`,
      `  - Decision Rule: If lead metric '${strategy.leadMetric}' drops below weekly threshold, reduce context switching inputs.`,
      "Pillar 3 (Evidence Loop):",
      `  - Setup: Log lead and lag metrics after each execution cycle.`,
      `  - Decision Rule: If lag metric '${strategy.lagMetric}' stalls for 2 weeks, revise workflow not effort intensity.`,
      "Application Guidance:",
      `  - When to Use: Use this model when your goal requires reliable compounding execution for ${GOAL_LABEL[goalId]}.`,
      `  - Expected Outcome: Better ${focus}, reduced '${strategy.riskPattern}', and visible metric improvement.`,
    ],
    profile.anchors,
    3,
  );
  const plan = enforceAnchors(
    [
      ...
      `  - Checklist: ${profile.weeklyPlan[0]}; set baseline for '${strategy.leadMetric}' and '${strategy.lagMetric}'.`,
      ...
      `  - Checklist: ${profile.weeklyPlan[1]}; remove one recurring friction source tied to '${strategy.riskPattern}'.`,
      ...
      `  - Checklist: ${profile.weeklyPlan[2]}; introduce one quality checkpoint linked to ${focus}.`,
      ...
      "Monthly Takeaway:",
      `  - Outcome should be a reusable system that continuously improves ${focus}.`,
    ],
    profile.anchors,
    3,
  );
  const mistakes = enforceAnchors(
    profile.baseMistakes.map(
      (line) =>
        `${line} Prevention: apply ${profile.frameworkName} plus ${derivedFrameworkName} decision rules consistently.`,
    ),
    profile.anchors,
    2,
  );
